...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [bitbucketPullrequestsPlugin],
    });

    await request(server).get('/api/bitbucket-pullrequests/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/bitbucket-pullrequests/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/bitbucket-pullrequests/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/bitbucket-pullrequests/todos/${createdTodoItem.id}`)
