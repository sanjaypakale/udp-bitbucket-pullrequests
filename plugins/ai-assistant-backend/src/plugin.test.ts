...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [aiAssistantPlugin],
    });

    await request(server).get('/api/ai-assistant/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/ai-assistant/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/ai-assistant/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/ai-assistant/todos/${createdTodoItem.id}`)
